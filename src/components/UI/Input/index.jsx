  ...
  return (
    <div className='flex flex-col items-start w-full'>
        ...
        <label className='px-2 font-medium' htmlFor={label.toLowerCase()}>{label}</label>
            <input type={type} name={label.toLowerCase()} value={value} id={label.toLowerCase()} placeholder={placeholder} className={`input2 flex p-1 px-2 w-full`} onChange={(e) => onChange( e.target.value )}/></div>
    </div>
  )
}
